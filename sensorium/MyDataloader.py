...
from neuralpredictors_cp.data.datasets import StaticImageSet, FileTreeDataset

from neuralpredictors_cp.data.transforms import (
    Subsample,
    ToTensor,
    NeuroNormalizer,
...
from neuralpredictors_cp.data.samplers import SubsetSequentialSampler
...
def static_loader(
    ...
    more_transforms.insert(0, AddBoundingBoxes())
    more_transforms.insert(0, AddCenteringImage())
    ...
    tier_attr = f"tiers_model_{model_id}" if model_id is not None else "tiers"
    tier_array = getattr(dat.trial_info, tier_attr) if file_tree else dat.tiers
