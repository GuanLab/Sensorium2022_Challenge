...
def generate_submission_file(
    trained_models, dataloaders, data_key=None, path=None, device="cpu", tier=None,
):
    """
    ...
    for tier in tier_list:
        ...
        for model in trained_models[1:]:
            cur_predictions = model_predictions(
                model,
                test_dataloader,
                data_key=data_key,
                device=device,
            )
            test_predictions += cur_predictions
        test_predictions /= len(trained_models)
