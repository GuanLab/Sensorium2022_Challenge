...
def get_correlations(
    models, dataloaders, tier=None, device="cpu", as_dict=False, per_neuron=True, **kwargs
):
    """
    Computes single-trial correlation between model prediction and true responses

    Args:
        models (list[torch.nn.Module]): Models used to predict responses.
    ...
    for k, v in dl.items():
        target, output = model_predictions(
            dataloader=v, model=models[0], data_key=k, device=device
        )
        for model in models[1:]:
            cur_target, cur_output = model_predictions(
                dataloader=v, model=model, data_key=k, device=device
            )
            assert (target == cur_target).all(), print("target changed!")
            output += cur_output
...
def get_signal_correlations(
    models, dataloaders, tier, device="cpu", as_dict=False, per_neuron=True
):
    """
    ...
    for data_key, dataloader in dataloaders[tier].items():
        ...
        _, predictions = model_predictions(
            models[0], dataloader, data_key=data_key, device=device
        )
        for model in models[1:]:
            _, cur_predictions = model_predictions(
...
def get_fev(models, dataloaders, tier, device="cpu", per_neuron=True, fev_threshold=0.15, as_dict=False):
    """
    Compute the fraction of explainable variance explained per neuron.

    Args:
        model (List[torch.nn.Module]): Models used to predict responses.
        dataloaders (dict): dict of test set torch dataloaders.
