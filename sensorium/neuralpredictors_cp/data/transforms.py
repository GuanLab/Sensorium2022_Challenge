...
class AddBehaviorAsChannels(MovieTransform, StaticTransform, Invertible):
    ...
    def __init__(self):
        ...
        self.transforms["xywh"] = lambda x: x
    ...
    def __call__(self, x):
        ...
        dd = {
            ...
            "xywh": self.transforms["xywh"](key_vals["xywh"])
        }
...
class AddCenteringImage(MovieTransform, StaticTransform, Invertible):
    """
    Given a StaticImage object that includes "images", "responses", and "behavior", it returns three variables:
        - input image concatinated with behavior as new channel(s)
        - responses
    ...
    def __init__(self):
        self.transforms, self.itransforms = {}, {}
        self.transforms["images"] = lambda img: np.concatenate(
            (
                img,
...
class AddBoundingBoxes(MovieTransform, StaticTransform, Invertible):
    """
    Given a StaticImage object that includes "images", "responses", and "behavior", it returns three variables:
        - input image concatinated with behavior as new channel(s)
        - responses
    ...
    def __init__(self):
        self.transforms, self.itransforms = {}, {}
        self.transforms["images"] = lambda img, xywh: np.concatenate(
            (
                img,
                np.ones((1, *img.shape[-(len(img.shape) - 1) :]))
                * np.expand_dims(xywh, axis=((len(img.shape) - 2), (len(img.shape) - 1))),
            ),
            axis=0,
        )
        self.transforms["responses"] = lambda x: x
    ...
    def __call__(self, x):
        ...
        key_vals = {k: v for k, v in zip(x._fields, x)}
        dd = {
            "images": self.transforms["images"](key_vals["images"], key_vals["xywh"]),
            "responses": self.transforms["responses"](key_vals["responses"]),
            "xywh": self.transforms["xywh"](key_vals["xywh"])
        }
        return x.__class__(**dd)